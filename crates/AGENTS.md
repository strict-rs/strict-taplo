...
- Return the concrete native assertion failure from each test, such as `Result<(), strict_test_support::ComparisonFailure<PathBuf, PathBuf>>`, or compose heterogeneous failures in a test-owned `thiserror` enum. The `strict-test-support` `src/lib.rs` module docs own the vocabulary and fixture contracts; apply the root testing contract when retaining subjects, fixture owners, and batch outcomes. Add `strict-test-support.workspace = true` under `[dev-dependencies]` (it is declared once in the root `[workspace.dependencies]`). Keep other fixtures crate-local.
