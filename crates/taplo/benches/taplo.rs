...
pub fn parsing(c: &mut Criterion) {
  ...
  c.bench_function("parse taplo syntax", |b| b.iter(|| parse(black_box(source))));
  c.bench_function("parse taplo dom", |b| b.iter(|| parse(black_box(source)).into_dom()));
  c.bench_function("parse taplo dom and validate", |b| {
    b.iter(|| parse(black_box(source)).into_dom().validate())
  });
  c.bench_function("parse toml-rs", |b| b.iter(|| toml::from_str::<toml::Value>(black_box(source))));
}
...
pub fn formatting(c: &mut Criterion) {
  ...
  c.bench_function("parse and format", |b| b.iter(|| format(black_box(source), Options::default())));
}
