...
impl core::fmt::Display for OptionParseError {
  fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
    write!(f, "invalid formatting option: {}", match self {
...
impl Options {
  ...
  fn newlines(&self, count: usize) -> impl Iterator<Item = &'static str> {
    repeat_n(self.newline(), usize::min(count, self.allowed_blank_lines + 1))
  }
...
pub fn format_with_scopes(dom: Node, options: Options, errors: &[TextRange], scopes: ScopedOptions) -> String {
  let c = Context {
  ...
  let mut s = format_dom(dom, options.clone(), c);
...
///
/// **This doesn't check errors of the DOM.**
pub fn format_with_path_scopes<I, S>(dom: Node, options: Options, errors: &[TextRange], scopes: I) -> Result<String, dom::Error>
where
...
fn format_impl(node: SyntaxNode, options: Options, context: Context) -> String {
  let mut formatted = if node.kind() == ROOT {
    format_root(node, &options, &context)
  } else {
    node.to_string()
  };
...
fn format_dom(dom: Node, options: Options, context: Context) -> String {
  dom
    .syntax()
    .and_then(|syntax| syntax.clone().into_node())
    .map_or_else(|| dom.to_toml(false, false), |syntax| format_impl(syntax, options, context))
}
...
impl FormattedEntry {
  fn cleaned_key(&self) -> &[String] {
    self
      .cleaned_key
      .get_or_init(|| self.key.replace(['\'', '"'], "").split('.').map(ToOwned::to_owned).collect())
  }
...
impl FormattedValue {
  ...
  fn new(text: String, trailing_comment: Option<String>) -> Self {
    Self {
      text,
      trailing_comment,
    }
...
fn format_root(node: SyntaxNode, options: &Options, context: &Context) -> String {
  ...
  fn add_comments(comments: &mut Vec<String>, formatted: &mut String, context: &Context, options: &Options) -> bool {
  ...
  for c in node.children_with_tokens() {
    ...
    match c {
      NodeOrToken::Node(node) => match node.kind() {
        TABLE_ARRAY_HEADER | TABLE_HEADER => {
          ...
          if let Some(key) = node.first_child().map(Into::into).map(Keys::from_syntax) {
            if scoped_options.indent_tables {
              context.indent_level = table_indent_level(&table_key_indent_history, &key, if scoped_options.indent_entries { 1 } else { 0 });
            }
          ...
          if add_comments(&mut comment_group, &mut formatted, &header_context, &scoped_options) {
            formatted += scoped_options.newline();
            skip_newlines = 0;
          }
          ...
          header.append_to(&mut formatted);
        ...
        _ => formatted.push_str(&node.to_string()),
      },
      NodeOrToken::Token(token) => match token.kind() {
        NEWLINE => {
          ...
          formatted.extend(scoped_options.newlines(newline_count.saturating_sub(skip_newlines)));
        }
        COMMENT => {
          if add_entries(&mut entry_group, &mut formatted, &scoped_options, &context) {
            formatted += scoped_options.newline();
...
fn table_indent_level(history: &[(Keys, usize)], current_key: &Keys, default_indent: usize) -> usize {
  history
    .iter()
    .rev()
    .find_map(|(previous_key, indent)| (current_key.contains(previous_key) && current_key != previous_key).then_some(*indent + 1))
    .unwrap_or(default_indent)
}

/// Add entries to the formatted string.
fn add_entries(entry_group: &mut Vec<FormattedEntry>, formatted: &mut String, options: &Options, context: &Context) -> bool {
    ...
    for entry in entry_group.iter_mut() {
      ...
      for (idx, line) in entry.value.split('\n').enumerate() {
        ...
        if chars_count > options.column_width {
          ...
          let Some(value_syntax) = entry.value_syntax.clone() else {
            continue;
          };
          let value = format_value(value_syntax, options, &context);
          ...
          value.append_to(&mut entry.value);
          break;
        }
      }
    }
    ...
    match c {
      NodeOrToken::Node(n) => match n.kind() {
        ...
        VALUE => {
          ...
          val.append_to(&mut value);
        }
        _ => value.push_str(&n.to_string()),
      },
      NodeOrToken::Token(t) => {
...
fn format_value(node: SyntaxNode, options: &Options, context: &Context) -> FormattedValue {
  ...
  FormattedValue::new(value, comment)
}
...
fn format_array(node: SyntaxNode, options: &Options, context: &Context) -> FormattedValue {
  ...
  let add_values =
    |value_group: &mut Vec<(String, Option<String>)>, commas_group: &mut Vec<bool>, formatted: &mut String, context: &Context| -> bool {
    ...
    match c {
      NodeOrToken::Node(n) => match n.kind() {
        VALUE => {
          ...
          val.append_to(&mut val_string);

          let has_comma = node_index < node_count - 1 || (multiline && options.array_trailing_comma);
        ...
        _ => {
          ...
          formatted.push_str(&n.to_string());
        }
      },
      NodeOrToken::Token(t) => match t.kind() {
        BRACKET_START => {
        ...
        COMMENT => {
          ...
          if !newline_before && !value_group.is_empty() {
            ...
            if let Some(last_value) = value_group.last_mut() {
              last_value.1 = Some(t.text().to_string());
            }
            continue;
          }

          if add_values(&mut value_group, &mut commas_group, &mut formatted, &inner_context) {
            formatted += options.newline();
            skip_newlines = 0;
          }
  ...
  FormattedValue::new(formatted, None)
}
  ...
  S: AsRef<str>,
{
  ...
  let diff_widths = |range: Range<usize>, row: &R| -> usize {
    ...
    for row in rows {
      ...
      let range = cmp::min(range.start, row_len.saturating_sub(1))..cmp::min(range.end, row_len);

      max_width = cmp::max(max_width, row.as_ref()[range].iter().map(|s| s.as_ref().chars().count()).sum());
    }

    let row_width = row.as_ref()[range].iter().map(|s| s.as_ref().chars().count()).sum::<usize>();
  ...
  for (row_idx, row) in rows.iter().enumerate() {
    ...
    for (item_idx, item) in row.as_ref().iter().enumerate() {
      if item_idx > separator_range.start && item_idx <= separator_range.end.saturating_add(1) && item_idx < row.as_ref().len() {
        out += separator;
      }

      out += item.as_ref();

      if can_align && align_range.start <= item_idx && align_range.end > item_idx && item_idx < row.as_ref().len() - 1 {
