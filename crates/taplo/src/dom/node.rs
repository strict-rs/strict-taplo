...
impl Node {
  pub fn path(&self, keys: &Keys) -> Option<Node> {
    ...
    if node.is_invalid() { None } else { Some(node) }
  }
  ...
  pub fn try_get(&self, idx: impl Index) -> Result<Node, Error> {
    idx.index_into(self).ok_or(Error::Query(QueryError::NotFound))
  }

  pub fn get_matches(&self, pattern: &str) -> Result<impl ExactSizeIterator<Item = (KeyOrIndex, Node)>, Error> {
  ...
  pub fn find_all_matches(&self, keys: Keys, include_children: bool) -> Result<impl ExactSizeIterator<Item = (Keys, Node)>, Error> {
  ...
  // (edition 2024), since callers invoke it on temporary nodes (e.g.
  // `node.get(key).text_ranges(..)`).
  ...
  /// All the comments in the tree, including header comments returned from
  ...
  pub fn header_comments(&self) -> impl Iterator<Item = Comment> {
    ...
    match first_item {
      Some(it) => Either::Left(
        self
          .comments()
          .take_while(move |c| c.syntax.as_ref().unwrap().text_range().end() <= it.text_range().start()),
      ),
...
impl Node {
  ...
  pub fn as_invalid(&self) -> Option<&Invalid> {
    if let Self::Invalid(v) = self { Some(v) } else { None }
  }

  pub fn try_into_table(self) -> Result<Table, Self> {
    if let Self::Table(v) = self { Ok(v) } else { Err(self) }
  }

  pub fn try_into_array(self) -> Result<Array, Self> {
    if let Self::Array(v) = self { Ok(v) } else { Err(self) }
  }

  pub fn try_into_bool(self) -> Result<Bool, Self> {
    if let Self::Bool(v) = self { Ok(v) } else { Err(self) }
  }

  pub fn try_into_str(self) -> Result<Str, Self> {
    if let Self::Str(v) = self { Ok(v) } else { Err(self) }
  }

  pub fn try_into_integer(self) -> Result<Integer, Self> {
    if let Self::Integer(v) = self { Ok(v) } else { Err(self) }
  }

  pub fn try_into_float(self) -> Result<Float, Self> {
    if let Self::Float(v) = self { Ok(v) } else { Err(self) }
  }

  pub fn try_into_date(self) -> Result<DateTime, Self> {
    if let Self::Date(v) = self { Ok(v) } else { Err(self) }
  }

  pub fn try_into_invalid(self) -> Result<Invalid, Self> {
    if let Self::Invalid(v) = self { Ok(v) } else { Err(self) }
  }
}
