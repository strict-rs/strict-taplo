...
use crate::util::iter::ExactIterExt;
...
impl FromSyntax for Node {
  fn from_syntax(syntax: SyntaxElement) -> Self {
    match syntax.kind() {
      ...
      STRING | MULTI_LINE_STRING | STRING_LITERAL | MULTI_LINE_STRING_LITERAL => Str::from_syntax(syntax).into(),
      INTEGER | INTEGER_HEX | INTEGER_OCT | INTEGER_BIN => Integer::from_syntax(syntax).into(),
      FLOAT => Float::from_syntax(syntax).into(),
      BOOL => Bool::from_syntax(syntax).into(),
      DATE_TIME_OFFSET | DATE_TIME_LOCAL | DATE | TIME => DateTime::from_syntax(syntax).into(),
    ...
    match syntax.kind() {
      ARRAY => ArrayInner {
        ...
        items:  Shared::new(
          syntax
            .as_node()
            .map(|n| n.children().map(|syntax| Node::from_syntax(syntax.into())).collect())
            .unwrap_or_default(),
        ),
      }
      .into(),
...
fn entry_from_syntax(syntax: SyntaxElement) -> (Key, Node) {
  ...
  let first_key = keys.next().unwrap_or_else(|| Key::from_syntax_invalid(syntax.clone()));
  ...
  for child in node.children() {
    match child.kind() {
      table_kind @ (TABLE_ARRAY_HEADER | TABLE_HEADER) => {
        let mut keys = keys_from_syntax(&child.first_child().map(Into::into).unwrap_or_else(|| child.clone().into()));
                ...
                match current_entries.lookup.get_key_value(&key) {
                  Some((k, Node::Table(t))) => {
                    if let Some(syntax) = key.syntax() {
                      k.inner.additional_syntaxes.update(|s| s.push(syntax.clone()));
                    }
                ...
                current_table.clone().inner.entries.update(|entries| {
                  if let Some((existing_key, existing_node)) = entries.lookup.get_key_value(&key) {
                    if let Some(key_syntax) = key.syntax() {
                      existing_key.inner.additional_syntaxes.update(|key_syntaxes| {
    ...
    if let Some((existing_key, existing_node)) = entries.lookup.get_key_value(&key) {
      ...
      match existing_node {
        Node::Table(existing_table) => {
          if !matches!(existing_table.inner.kind, TableKind::Regular | TableKind::Pseudo) {
        ...
        Node::Array(existing_array) => {
          ...
          if let Some(Node::Table(t)) = item {
            ...
            existing_array.inner.items.update(|items| items.push(pt.clone().into()));
            current_table = pt;
          }
        }
        _ => {
