...
impl Node {
  pub fn to_toml(&self, inline: bool, prefer_single_quote: bool) -> String {
    ...
    self.to_toml_fmt(&mut s, inline, prefer_single_quote).unwrap();
    s
  }

  pub fn to_toml_fmt(&self, f: &mut impl Write, inline: bool, prefer_single_quote: bool) -> core::fmt::Result {
  ...
  fn to_toml_impl(
    ...
    if let Node::Bool(_) | Node::Str(_) | Node::Integer(_) | Node::Float(_) | Node::Date(_) = self {
      ...
      Node::Table(table) => {
        if table.inner.kind == TableKind::Inline || inline {
          ...
          for (key, node) in entries
            .iter()
            .filter(|(_, n)| !n.is_table() && !n.as_array().map(|n| n.inner.kind == ArrayKind::Tables).unwrap_or(false))
          {
            node.to_toml_impl(f, key.clone().into(), false, false, prefer_single_quote)?;
            f.write_char('\n')?;
          }
          ...
          for (key, node) in entries
            ...
            .filter(|(_, n)| n.is_table() || n.as_array().map(|n| n.inner.kind == ArrayKind::Tables).unwrap_or(false))
          {
            node.to_toml_impl(f, parent_keys.join(key.clone()), false, false, prefer_single_quote)?;
          }
        }
      }
      Node::Array(array) => {
        if array.inner.kind == ArrayKind::Inline || inline {
          ...
          for node in items.iter() {
            ...
            f.write_str(parent_keys.dotted())?;
            f.write_str("]]\n")?;
            node.to_toml_impl(f, parent_keys.clone(), false, true, prefer_single_quote)?;
          }
        }
      }
