...
impl Keys {
  ...
  pub fn all_text_range(&self) -> TextRange {
    join_ranges(self.keys.iter().filter_map(KeyOrIndex::as_key).flat_map(|k| k.text_ranges()))
  }
}
...
impl FromStr for Keys {
  ...
  fn from_str(s: &str) -> Result<Self, Self::Err> {
    ...
    Ok(Keys::new(keys_from_syntax(&p.into_syntax().into()).map(Into::into)))
  }
}
