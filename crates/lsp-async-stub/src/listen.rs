...
pub(crate) fn create_output(sink: impl AsyncWrite + Unpin + Send + 'static) -> UnboundedSender<rpc::Message> {
...
pub(crate) async fn write_message<T: AsyncWrite + Unpin>(out: &mut T, msg: rpc::Message) -> anyhow::Result<()> {
...
pub(crate) fn create_input(stream: impl AsyncRead + Unpin + Send + 'static) -> UnboundedReceiver<rpc::Message> {
...
pub(crate) async fn read_message<R: tokio::io::AsyncBufRead + Unpin>(mut input: R) -> Result<Option<rpc::Message>, anyhow::Error> {
  let mut size = 0;
  ...
  loop {
    ...
    let header_name = parts.next().ok_or_else(|| anyhow!("invalid header: {}", buf))?;
