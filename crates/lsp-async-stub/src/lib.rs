...
use futures::{
    channel::oneshot,
    future::{FusedFuture, LocalBoxFuture},
    lock::Mutex as AsyncMutex,
    sink::Sink,
    Future, FutureExt, SinkExt,
};
use handler::{ErasedNotificationHandler, ErasedRequestHandler, RequestOutcome};
use lsp_types::{
    notification::{self, Notification},
    request as request_types,
    request::Request,
    NumberOrString,
};
    ...
    >(
        ...
        let response = response.map_err(|_| {
            io::Error::new(
                ...
                "outbound request response channel was dropped",
            )
        })?;
...
struct SessionState {
    ...
    pending_requests:
        HashMap<rpc::RequestId, oneshot::Sender<rpc::Response<serde_json::Value>>>,
}
...
impl<W: Clone + 'static> Server<W> {
        ...
        F: Future<Output = ()> + 'static,
    {
        self.notification_handlers.insert(
            N::METHOD,
            Rc::new(handler::NotificationHandler::<N, _, _>::new(handler)),
        );
        ...
        F: Future<Output = Result<R::Result, rpc::Error>> + 'static,
    {
        self.request_handlers.insert(
            R::METHOD,
            Rc::new(handler::RequestHandler::<R, _, _>::new(handler)),
        );
            ...
            return writer
                .send(error_message(
                    ...
                    error_with_text(
                        ...
                        "only JSON-RPC version 2.0 is accepted",
                    ),
                ))
                .await;
        }
...
async fn run_deferred<W: Clone>(context: &Context<W>, method: &str) {
    ...
    for task in deferred {
        task.instrument(tracing::trace_span!("deferred task", %method))
            .await;
    }
}
...
fn outcome_message(id: rpc::RequestId, outcome: RequestOutcome) -> rpc::Message {
    match outcome {
        ...
        RequestOutcome::InvalidParams(detail) => {
            error_message(id, error_with_text(rpc::Error::invalid_params(), detail))
        }
        RequestOutcome::SerializationFailure(detail) => {
            error_message(id, error_with_text(rpc::Error::internal_error(), detail))
        }
    }
}
...
impl<P> Params<P> {
    ...
    pub fn required(self) -> Result<P, rpc::Error> {
        self.0.ok_or_else(|| {
            error_with_text(rpc::Error::invalid_params(), "params are required")
        })
    }
}
...
mod tests {
    use super::{
        lock_recovering_poison, request_message, CancelToken, Cancellation, Context, MessageWriter,
        Params, RequestWriter, Server, ServerBuilder,
    };
    use crate::rpc;
    use futures::{
        channel::oneshot,
    ...
    impl TestWorld {
        ...
        fn record_request(&self) {
            self.requests
                .set(self.requests.get().saturating_add(1));
        }
    ...
    async fn initialize_handler(
        context: Context<TestWorld>,
        params: Params<TestParams>,
    ) -> Result<TestResult, rpc::Error> {
        let params = params.required()?;
        context.record_request();
        Ok(TestResult { value: params.value })
    }
    ...
    async fn serialization_handler(
        context: Context<TestWorld>,
        params: Params<TestParams>,
    ) -> Result<FailingResult, rpc::Error> {
        let _ = params.required()?;
        context.record_request();
        Ok(FailingResult)
    }
    ...
    async fn cancellable_handler(
        mut context: Context<TestWorld>,
        params: Params<TestParams>,
    ) -> Result<TestResult, rpc::Error> {
        let params = params.required()?;
        context.record_request();
    ...
    async fn token_observing_handler(
        mut context: Context<TestWorld>,
        params: Params<TestParams>,
    ) -> Result<TestResult, rpc::Error> {
        let params = params.required()?;
    ...
    async fn shutdown_handler(
        _context: Context<TestWorld>,
        _params: Params<()>,
    ) -> Result<(), rpc::Error> {
        Ok(())
    }

    async fn failing_initialize_handler(
        _context: Context<TestWorld>,
        _params: Params<TestParams>,
    ) -> Result<TestResult, rpc::Error> {
        Err(rpc::Error::internal_error())
    }

    async fn failing_shutdown_handler(
        _context: Context<TestWorld>,
        _params: Params<()>,
    ) -> Result<(), rpc::Error> {
        Err(rpc::Error::internal_error())
    ...
    fn last_message(sink: &TestSink) -> Result<rpc::Message, TestFailure> {
        ensure_some(
            sink.messages().last().cloned(),
            "the server must emit a response message",
        )
    }

    fn initialize(
        server: &Server<TestWorld>,
        world: &TestWorld,
        sink: &TestSink,
    ) -> Result<(), TestFailure> {
        ensure_ok(
    ...
    fn builder_preserves_kind_separated_registration_and_dispatch() -> Result<(), TestFailure> {
        ...
        ensure_ok(
            futures::executor::block_on(server.handle_message(
                world.clone(),
                request_message("test/shared", None, params(8)),
                sink.clone(),
            )),
        ...
        ensure_eq(
            ...
            "a valid notification must emit no response",
        )?;
        ...
        ensure_eq(
            ...
            sink.messages().is_empty(),
            "a request-only notification must emit no response",
        )
    }

    #[test]
    fn request_outcomes_preserve_ids_and_do_not_invoke_invalid_handlers(
    ) -> Result<(), TestFailure> {
        ...
        ensure_eq(
            &world.requests.get(),
            &1,
            "invalid params must not invoke the typed handler",
        )?;
    ...
    fn lifecycle_transitions_only_after_successful_results_and_writes() -> Result<(), TestFailure> {
        ...
        ensure_ok(
            ...
            !futures::executor::block_on(async {
                failed_initialize.session.lock().await.initialized
            }),
            "an initialize handler error must not transition state",
        )?;

        let writer_failed_initialize = Server::new()
            .on_request::<InitializeRequest, _>(initialize_handler)
            .build();
        ...
        let writer_result = futures::executor::block_on(writer_failed_initialize.handle_message(
            ...
            "initialize response writer failure must propagate",
        )?;
        ensure(
            !futures::executor::block_on(async {
                writer_failed_initialize.session.lock().await.initialized
            }),
        ...
        ensure_ok(
            futures::executor::block_on(server.handle_message(
                world.clone(),
                request_message("shutdown", Some(id(6)), None),
                live_sink.clone(),
            )),
        ...
        ensure_ok(
            futures::executor::block_on(server.handle_message(
                world,
                request_message("test/basic", Some(id(7)), params(7)),
                live_sink.clone(),
            )),
    ...
    #[test]
    fn failed_shutdown_and_writer_failure_do_not_run_request_deferred_work(
    ) -> Result<(), TestFailure> {
        let world = TestWorld::default();
        let sink = TestSink::default();
        ...
        let failed_write = futures::executor::block_on(deferred_server.handle_message(
            ...
            failed_write.is_err(),
            "request response writer failure must propagate",
        )?;
        ensure_eq(
            &world.deferred.get(),
            &0,
            "request-deferred work must not run after a failed send",
        )?;
        ...
        ensure_eq(
            &world.deferred.get(),
            &2,
            "notification-deferred work must run after its handler",
        )
    }

    #[test]
    fn duplicate_ids_and_cancellation_affect_only_the_matching_active_task(
    ) -> Result<(), TestFailure> {
        let server = Server::new()
            .on_request::<InitializeRequest, _>(initialize_handler)
        ...
        ensure_eq(
            &world.requests.get(),
            &2,
            "initialize plus one active cancellable handler must run",
        )?;
        ...
        ensure_eq(
            &world.requests.get(),
            &2,
            "a duplicate active ID must not invoke a second handler",
        )?;
        ensure(
    ...
    #[test]
    fn normal_completion_removes_task_without_cancelling_retained_token(
    ) -> Result<(), TestFailure> {
        let server = Server::new()
            .on_request::<InitializeRequest, _>(initialize_handler)
        ...
        ensure_ok(
            futures::executor::block_on(server.handle_message(
                world.clone(),
                request_message("test/basic", Some(id(2)), params(2)),
                sink,
            )),
        ...
        let token = ensure_some(
            ...
            futures::executor::block_on(async {
                server.session.lock().await.active_tasks.is_empty()
            }),
    ...
    fn outbound_request_failures_are_fallible_and_do_not_wrap_ids() -> Result<(), TestFailure> {
        ...
        let exhausted = futures::executor::block_on(
            context.write_request::<BasicRequest, _>(Some(TestParams { value: 1 })),
        );
        let exhausted_error = ensure_some(exhausted.err(), "ID exhaustion must return an error")?;
        ensure(
            exhausted_error.kind() == io::ErrorKind::Other,
            "ID exhaustion error kind",
        )?;
        ensure(
            sink.messages().is_empty(),
            "ID exhaustion must not emit a wrapped request ID",
        )?;
        ...
        ensure_ok(
            pool.spawner().spawn_local(async move {
                let result = dropped_context
                    .write_request::<BasicRequest, _>(Some(TestParams { value: 2 }))
                    .await;
        ...
        let dropped = ensure_ok(
            pool.run_until(result_receiver),
            "the outbound result channel must remain connected",
        )?;
        ...
        ensure_ok(
            pool.spawner().spawn_local(async move {
                let result = malformed_context
                    .write_request::<BasicRequest, _>(Some(TestParams { value: 3 }))
                    .await;
        ...
        pool.run_until(super::Server::<TestWorld>::handle_response(
        ...
        let malformed_error = ensure_some(
            malformed.err(),
            "malformed response deserialization must return an error",
        )?;
        ensure(
    ...
    #[test]
    fn malformed_wire_data_and_empty_response_results_are_typed_errors(
    ) -> Result<(), TestFailure> {
        let server = Server::<TestWorld>::new().build();
        ...
        let wire_error = futures::executor::block_on(server.handle_message(
            TestWorld::default(),
            invalid,
            TestSink::default(),
        ));
        let wire_error = ensure_some(wire_error.err(), "invalid wire data must return an error")?;
            ...
            wire_error.kind() == io::ErrorKind::InvalidData,
            "invalid wire data error kind",
        )?;
        ...
        let response_error = ensure_some(
            response.into_result().err(),
            "a response without result or error must be rejected",
        )?;
        ensure_eq(
            &response_error.code,
            &-32603,
            "empty response internal-error code",
        )
    }
}
