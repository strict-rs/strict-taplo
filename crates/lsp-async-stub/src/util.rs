...
pub struct Position {
  ...
  pub line:      u64,
  /// Character offset on a line in a document (could be zero-based or one-based based on the
  /// usage).
  pub character: u64,
}
...
impl Mapper {
  ...
  pub fn range(&self, range: TextRange) -> Option<Range> {
    self.position(range.start()).and_then(|start| {
      self.position(range.end()).map(|end| Range {
        start,
        end,
      })
    })
  ...
  fn new_impl(source: &str, utf16: bool, base: u64) -> Self {
    ...
    for c in source.chars() {
      ...
      offset_to_position.extend((last_offset..new_offset).map(|b| {
        (TextSize::from(b as u32), Position {
          line,
          character,
        })
      }));
    ...
    offset_to_position.insert(TextSize::from(last_offset as u32), Position {
      line,
      character,
    });
...
line-3"#;
  ...
  assert!(
    mapper.position(TextSize::from(s1.len() as u32 - 1_u32)).unwrap()
      == Position {
        line:      2,
        character: 5,
      }
