...
use crate::Server;
use crate::listen::create_input;
use crate::listen::create_output;

impl<W: Clone + 'static> Server<W> {
  pub async fn listen_tcp<A>(self, world: W, address: A, mut shutdown_signals: impl Stream<Item = ()> + Unpin) -> Result<(), anyhow::Error>
  where
    A: ToSocketAddrs,
  {
    ...
    self.listen_loop(world, input, output, shutdown_signals).await
  }
}
