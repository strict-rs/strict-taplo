...
use crate::environment::js_io_error;
...
impl Sink<rpc::Message> for WasmLspInterface {
  ...
  fn start_send(self: std::pin::Pin<&mut Self>, message: rpc::Message) -> Result<(), Self::Error> {
    ...
    let message = serde_wasm_bindgen::to_value(&message).map_err(|error| io::Error::new(io::ErrorKind::InvalidData, error))?;
