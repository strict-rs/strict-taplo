...
pub async fn lint(env: JsValue, toml: String, config: JsValue) -> Result<JsValue, JsError> {
  ...
  if let Some(schema) = schemas.associations().association_for(&Url::parse("file:///__.toml").unwrap()) {
...
pub async fn run_cli(env: JsValue, args: JsValue) -> Result<(), JsError> {
  ...
  setup_stderr_logging(env.clone(), cli.log_spans, cli.verbose, match cli.colors {
    Colors::Auto => None,
