...
impl<E: Environment> Taplo<E> {
  pub fn new(env: E) -> Self {
    ...
    let http = taplo_common::util::get_reqwest_client(std::time::Duration::from_secs(5)).unwrap();
  ...
  async fn collect_files(
    ...
    if patterns.is_empty() {
      patterns = match config.include.clone() {
        ...
        None => Vec::from([cwd.join("**/*.toml").normalize().to_string_lossy().into_owned()]),
      };
    };
    ...
    let files = files.into_iter().filter(|path| config.is_included(path)).collect::<Vec<_>>();
