...
impl<E: Environment> Taplo<E> {
    ...
    if !cmd.no_schema {
      if let Some(schema_url) = cmd.schema.clone() {
        self
          .schemas
          .associations()
          .add(AssociationRule::regex(".*")?, SchemaAssociation {
  ...
  async fn lint_files(&mut self, cmd: LintCommand) -> Result<(), anyhow::Error> {
    ...
    let files = self.collect_files(&cwd, &config, cmd.files.into_iter()).await?;
  ...
  async fn lint_source(&self, file_path: &str, source: &str) -> Result<(), anyhow::Error> {
    ...
    self
      .print_parse_errors(&SimpleFile::new(file_path, source), &parse.errors)
      .await?;
    ...
    if let Err(errors) = dom.validate() {
      self.print_semantic_errors(&SimpleFile::new(file_path, source), errors).await?;
    ...
    self.schemas.associations().add_from_document(&file_uri, &dom);
    ...
    if let Some(schema_association) = self.schemas.associations().association_for(&file_uri) {
      ...
      let errors = self.schemas.validate_root(&schema_association.url, &dom).await?;

      if !errors.is_empty() {
        self.print_schema_errors(&SimpleFile::new(file_path, source), &errors).await?;
