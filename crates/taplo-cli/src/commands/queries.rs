...
impl<E: Environment> Taplo<E> {
  pub async fn execute_get(&self, cmd: GetCommand) -> Result<(), anyhow::Error> {
    ...
    if cmd.separator.is_some() && !matches!(cmd.output_format, OutputFormat::Value) {
      return Err(anyhow!("`--separator` is only valid for `--output-format value`"));
    }
    ...
    if let Err(errors) = node.validate() {
      self
        .print_semantic_errors(&SimpleFile::new(&file_path, &source), errors)
        .await?;
    ...
    match cmd.output_format {
      crate::args::OutputFormat::Json => {
        if let Some(p) = cmd.pattern {
          ...
          let keys = p.parse::<Keys>().map_err(|err| anyhow!("invalid pattern: {err}"))?;
          ...
          if nodes.len() == 1 {
            stdout.write_all(&serde_json::to_vec_pretty(&nodes.next().unwrap().1)?).await?;
            ...
            stdout
              .write_all(&serde_json::to_vec_pretty(&nodes.map(|n| n.1).collect::<Vec<_>>())?)
              .await?;
            if !cmd.strip_newline {
              stdout.write_all(b"\n").await?;
            }
