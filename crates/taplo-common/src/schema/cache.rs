...
impl<E: Environment> Cache<E> {
  pub fn new(env: E) -> Self {
    Self {
      ...
      lru_expires_by: Arc::new(Mutex::new(expiration_deadline(env.now(), DEFAULT_LRU_CACHE_EXPIRATION_TIME))),
      env,
  ...
  pub async fn load(&self, value_url: &Url, include_expired: bool) -> Result<Arc<Value>, anyhow::Error> {
  ...
  pub(crate) async fn save_if_configured(&self, url: Url, value: Arc<Value>) -> Result<(), anyhow::Error> {
    let disk_root = self.disk_root.load_full();
    match disk_root.as_ref() {
      Some(disk_root) => self.save_at(disk_root, url, value).await,
      None => Ok(()),
  ...
  async fn save_at(&self, disk_root: &Path, url: Url, value: Arc<Value>) -> Result<(), anyhow::Error> {
  ...
  pub fn lru_expired(&self) -> bool {
    ...
    if expired {
      *(self.lru_expires_by.lock()) = expiration_deadline(now, self.expiration_policy.load().memory);
    }
    expired
  }
}
...
mod tests {
  ...
  use super::DEFAULT_CACHE_EXPIRATION_TIME;
  use super::DEFAULT_LRU_CACHE_EXPIRATION_TIME;
  ...
  fn optional_and_required_disk_persistence_have_distinct_contracts() -> Result<(), TestFailure> {
    futures::executor::block_on(async {
      ...
      ensure_anyhow(
        cache.save_if_configured(url.clone(), value.clone()).await,
      ...
      ensure(cached.value == *value, "the configured write must preserve the schema value")?;
  ...
  fn default_policy_and_memory_expiration_have_both_polarities() -> Result<(), TestFailure> {
    futures::executor::block_on(async {
      ...
      ensure(!cache.lru_expired(), "memory entries must remain live before the deadline")?;
      ensure(cache.contains_schema(&url), "a pre-deadline expiration check must retain the entry")?;
      ...
      ensure(load_result.is_err(), "an expired memory entry with no disk fallback must not load")?;
      ensure(
