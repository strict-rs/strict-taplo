...
impl Config {
  ...
  pub fn prepare(&mut self, e: &impl Environment, base: &Path) -> Result<(), anyhow::Error> {
    ...
    self.file_rule = Some(GlobRule::new(
      self.include.as_deref().unwrap_or(&[default_include] as &[String]),
  ...
  pub fn is_schema_enabled(&self, path: &Path) -> bool {
    let enabled = self.global_options.schema.as_ref().and_then(|s| s.enabled).unwrap_or(true);
    ...
    if let Some(included) = &mut self.include {
      for pat in included {
        if !e.is_absolute(Path::new(pat)) {
          *pat = base.join(pat.as_str()).normalize().to_string_lossy().into_owned();
        }
      }
    }
