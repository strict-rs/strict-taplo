...
impl Query {
  ...
  pub fn in_table_header(&self) -> bool {
    match (&self.before, &self.after) {
      (Some(before), Some(after)) => {
        let Some(header_syntax) = before.syntax.parent_ancestors().find(|s| s.kind() == TABLE_HEADER) else {
          return false;
        };
  ...
  pub fn in_table_array_header(&self) -> bool {
    match (&self.before, &self.after) {
      (Some(before), Some(after)) => {
        let Some(header_syntax) = before.syntax.parent_ancestors().find(|s| s.kind() == TABLE_ARRAY_HEADER) else {
          return false;
        };
  ...
  pub fn is_inline(&self) -> bool {
    ...
    syntax.parent_ancestors().any(|a| matches!(a.kind(), INLINE_TABLE | ARRAY))
  }
  ...
  pub fn entry_keys(&self) -> Keys {
    self.entry_key().map_or_else(Keys::empty, |keys| Keys::from_syntax(keys.into()))
  }
...
mod tests {
  ...
  use taplo::syntax::SyntaxKind::PERIOD;
