...
pub(crate) async fn format<E: Environment>(
  ...
  let doc_path = context
    ...
    .ok_or_else(|| Error::invalid_request().with_data(format!("invalid (non-local) uri for file: {document_uri}")))?;

  let tab_size = usize::try_from(p.options.tab_size).map_err(|_| Error::invalid_params().with_data("tab size is not representable"))?;
  ...
  snapshot.taplo_config.update_format_options(&doc_path, &mut format_opts);
  ...
  let range = crate::uri::mapper_range_to_lsp(doc.mapper.all_range())
    .ok_or_else(|| Error::internal_error().with_data("document range is not representable by LSP"))?;
  ...
  Ok(Some(vec![TextEdit {
    ...
    new_text: taplo::formatter::format_with_path_scopes(
      doc.dom.clone(),
      format_opts,
      &doc.parse.errors.iter().map(|err| err.range).collect::<Vec<_>>(),
      scopes.into_iter(),
    )
    .map_err(|err| {
