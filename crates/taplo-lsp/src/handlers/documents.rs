...
use crate::{
    diagnostics,
    world::{
        send_association_notifications, DocumentState, WorkspaceState, World,
    },
};
...
fn replace_document<E: Environment>(
    ...
    workspace
        .documents
        .insert(document_url.clone(), document.clone());
    workspace
        .schemas
        .associations()
...
async fn publish_excluded<E: Environment>(
...
#[cfg(test)]
mod tests {
    use super::{parse_document, remove_document, replace_document};
    use crate::{
        test_support::{ensure_anyhow, TestEnvironment},
        world::{WorkspaceRoot, WorkspaceState},
    };
    use std::path::Path;
    ...
    #[test]
    fn replacement_refreshes_derived_ownership_and_preserves_manual_associations(
    ) -> Result<(), TestFailure> {
        ...
        workspace.schemas.associations().add(
        ...
        let _ = replace_document(
            ...
            &document_url,
            parse_document(
                "\"$schema\" = \"https://example.com/field.json\"\nvalue = 2\n",
            ),
        );
        ensure_eq(
            &source_count(&workspace, source::DIRECTIVE),
    ...
    #[test]
    fn disablement_hides_but_retains_derived_state_and_close_removes_only_that_state(
    ) -> Result<(), TestFailure> {
        let environment = TestEnvironment::default();
        let mut workspace = WorkspaceState::new(
            environment.clone(),
    ...
    #[test]
    fn exclusion_removes_file_state_but_never_applies_to_non_file_urls(
    ) -> Result<(), TestFailure> {
        let environment = TestEnvironment::default();
        let mut workspace = WorkspaceState::new(
            environment.clone(),
