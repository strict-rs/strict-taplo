...
use lsp_types::{
    Position, PrepareRenameResponse, RenameParams, TextDocumentPositionParams, TextEdit,
    WorkspaceEdit,
};
use std::collections::HashMap;
use taplo::{
    dom::{rewrite::Rewrite, KeyOrIndex, Keys},
    syntax::SyntaxKind,
};
...
fn prepare_document(
    ...
    let Some(offset) = document
        .mapper
        .offset(crate::uri::from_lsp_position(position))
    else {
        return Ok(None);
    };
    ...
    let Some(position) = query.first_matching(|position| position.syntax.kind() == SyntaxKind::IDENT)
    else {
        return Ok(None);
    };
...
pub async fn rename<E: Environment>(
    ...
    let params = params.required()?;
    let Some(document_uri) = crate::uri::to_url(&params.text_document_position.text_document.uri)
    else {
        return Ok(None);
    };
    let Some(snapshot) = context.document_snapshot(&document_uri).await else {
...
fn rename_document(
    ...
    let mut rewrite = Rewrite::new(document.dom.clone()).map_err(|error| {
        Error::internal_error().with_data(format!("failed to initialize rename rewrite: {error}"))
    })?;
    rewrite
        .rename_keys(keys.dotted(), new_name)
        .map_err(|error| {
            Error::internal_error().with_data(format!("failed to rename key: {error}"))
        })?;
    ...
    for patch in rewrite.patches() {
        ...
        let Some(range) = crate::uri::to_lsp_range(&document.mapper, patch.range) else {
            return Err(
                Error::internal_error().with_data("rename range is not representable by LSP")
            );
        };
        edits.push(TextEdit {
            range,
...
#[cfg(test)]
mod tests {
    use super::{prepare_document, rename_document};
    ...
    fn document_url() -> Result<Url, TestFailure> {
        ensure_ok(
            Url::parse("file:///workspace/file.toml"),
            "the rename fixture URL must parse",
        )
    }

    #[test]
    fn prepare_and_rename_accept_identifiers_but_reject_other_positions(
    ) -> Result<(), TestFailure> {
        let source = "alpha = 1\n";
        ...
        let prepared = ensure_some(
            ...
            prepared
                == PrepareRenameResponse::Range(lsp_types::Range::new(
                Position::new(0, 0),
                Position::new(0, 5),
            )),
    ...
    #[test]
    fn malformed_headers_and_missing_mapper_endpoints_do_not_return_partial_edits(
    ) -> Result<(), TestFailure> {
        ...
        let error = ensure_some(
            result.err(),
            "an unmappable replacement endpoint must return a typed error",
        )?;
        ensure_eq(
            &error.code,
            &-32603,
            "mapping failure must be reported as an internal RPC error",
        )?;
        ensure(
            error
                .data
