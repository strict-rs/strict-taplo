...
use taplo_common::schema::ValueExt;
...
pub async fn completion<E: Environment>(
  ...
  let Some(document_uri) = crate::uri::to_url(&p.text_document_position.text_document.uri) else {
    return Ok(None);
  };
  ...
  if query.in_table_header() {
    ...
    let object_schemas = match snapshot
      ...
      .map(|s| {
        s.into_iter().filter(|(_, _, s)| {
          s["type"].is_null() || s["type"] == "object" || s["type"].as_array().is_some_and(|arr| arr.iter().any(|v| v == "object"))
        })
      }) {
      Ok(s) => s,
      Err(error) => {
    ...
    let key_range = query.header_key().map(|key| key.text_range()).filter(|range| !range.is_empty());
    let key_lsp_range = key_range.and_then(|range| crate::uri::to_lsp_range(&doc.mapper, range));

    let node = query.dom_node().cloned().unwrap_or_else(|| (Keys::empty(), doc.dom.clone()));
  ...
  if query.in_table_array_header() {
    ...
    let array_of_objects_schemas = match snapshot
      ...
      .map(|s| {
        s.into_iter()
          .filter(|(_, _, s)| s["type"] == "array" && (s["items"]["type"] == "object" || s["items"]["type"].is_null()))
      }) {
      Ok(s) => s,
      Err(error) => {
        tracing::error!(?error, "failed to collect schemas");
  ...
  if query.in_entry_keys() {
    ...
    let key_range = query.entry_key().map(|key| key.text_range()).filter(|range| !range.is_empty());
    let key_lsp_range = key_range.and_then(|range| crate::uri::to_lsp_range(&doc.mapper, range));
  ...
  if query.in_entry_value() {
    ...
    for (_, _, schema) in schemas {
      add_value_completions(&schema, range, &mut completions, query.is_single_quote_value());
    }
...
fn schema_value_to_toml(value: &Value, single_quote: bool) -> Option<(String, CompletionItemKind)> {
  if value.is_null() {
    return None;
  }
...
fn value_completion(text: String, kind: CompletionItemKind, docs: Option<String>, range: Option<Range>) -> CompletionItem {
  CompletionItem {
    label: text.clone(),
    kind: Some(kind),
...
fn snippet_completion(label: &str, text: &str, docs: Option<String>, range: Option<Range>) -> CompletionItem {
  CompletionItem {
    label: label.into(),
...
fn add_value_completions(schema: &Value, range: Option<Range>, completions: &mut Vec<CompletionItem>, single_quote: bool) {
  ...
  let schema_docs = ext_docs.main.or_else(|| schema["description"].as_str().map(Into::into));
  ...
  if let Some(enum_values) = schema["enum"].as_array() {
    let enum_completions = enum_values
      ...
      .filter_map(|(index, value)| {
        ...
        let docs = enum_docs.get(index).cloned().flatten().or_else(|| schema_docs.clone());
  ...
  if let Some((text, kind)) = schema.get("const").and_then(|value| schema_value_to_toml(value, single_quote)) {
...
fn default_value_snippet(schema: &Value, cursor_count: usize, single_quote: bool) -> Cow<'static, str> {
  if let Some((text, _)) = schema.get("const").and_then(|value| schema_value_to_toml(value, single_quote)) {
  ...
  if schema["enum"]
    .as_array()
    .is_some_and(|values| values.iter().any(|value| schema_value_to_toml(value, single_quote).is_some()))
  {
    return format!("${cursor_count}").into();
  }

  let mut init_keys = schema_ext_of(schema).and_then(|ext| ext.init_keys).unwrap_or_default();

  if let Some(required) = schema["required"].as_array() {
    init_keys.extend(required.iter().filter_map(|value| value.as_str().map(ToOwned::to_owned)));
  }
  ...
  if !init_keys.is_empty() {
    ...
    for (index, init_key) in init_keys.iter().enumerate() {
      ...
      snippet.push_str(" = ");
      snippet.push_str(&default_value_snippet(&schema["properties"][init_key], nested_cursor, single_quote));
    }
...
mod tests {
  ...
  fn completion_documentation_prefers_extension_content() -> Result<(), TestFailure> {
    ...
    let docs = ensure_some(documentation(&schema), "completion docs must exist")?;
      ...
      markdown(&documentation(&json!({ "description": "description docs" }))) == Some("description docs"),
  ...
  fn schema_value_conversion_preserves_value_kind() -> Result<(), TestFailure> {
    let scalar = ensure_some(schema_value_to_toml(&json!(1), false), "an integer schema value must convert")?;
    ensure(
  ...
  fn recursive_default_snippets_use_safe_precedence_and_saturating_cursors() -> Result<(), TestFailure> {
    ensure_eq(
      &default_value_snippet(&json!({ "const": "fixed", "default": "other", "type": "string" }), 0, false).as_ref(),
      &r#"${0:"fixed"}"#,
      "valid const must win over default and type snippet",
    )?;
    ensure_eq(
      &default_value_snippet(&json!({ "const": null, "default": true, "type": "string" }), 0, false).as_ref(),
      &"${0:true}",
      "null const must fall through to a valid default",
    )?;
    ensure_eq(
      &default_value_snippet(&json!({ "enum": [null, 1], "type": "string" }), 0, false).as_ref(),
      &"$0",
      "an enum placeholder must be used only when at least one value converts",
    )?;
    ensure_eq(
      &default_value_snippet(&json!({ "enum": [null], "type": "string" }), 0, false).as_ref(),
      &r#""$0""#,
