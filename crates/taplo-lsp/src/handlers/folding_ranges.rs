...
use taplo::syntax::SyntaxKind::COMMENT;
use taplo::syntax::SyntaxKind::MULTI_LINE_STRING;
use taplo::syntax::SyntaxKind::MULTI_LINE_STRING_LITERAL;
use taplo::syntax::SyntaxKind::NEWLINE;
use taplo::syntax::SyntaxKind::TABLE_ARRAY_HEADER;
use taplo::syntax::SyntaxKind::TABLE_HEADER;
use taplo::syntax::SyntaxKind::WHITESPACE;
...
pub(crate) async fn folding_ranges<E: Environment>(
  ...
  Ok(Some(create_folding_ranges(syntax, &snapshot.document.mapper)))
}
...
pub fn create_folding_ranges(syntax: &SyntaxNode, mapper: &Mapper) -> Vec<FoldingRange> {
  ...
  for element in syntax.children_with_tokens() {
    ...
    match &element {
      ...
      SyntaxElement::Token(token) if token.kind() == NEWLINE && comment_start.is_some() && token.text().matches('\n').count() == 1 => {}
      _ => flush_comment_block(mapper, &mut comment_start, &mut last_comment, &mut ranges),
    }
...
fn collect_multiline_value_ranges(node: &SyntaxNode, mapper: &Mapper, ranges: &mut Vec<FoldingRange>) {
  for descendant in node.descendants_with_tokens() {
    match descendant {
      SyntaxElement::Node(array) if array.kind() == ARRAY && array.descendants_with_tokens().any(|element| element.kind() == NEWLINE) => {
        if let Some(range) = fold_range(mapper, array.text_range(), FoldingRangeKind::Region, true) {
          ranges.push(range);
        }
      }
      SyntaxElement::Token(string)
        if matches!(string.kind(), MULTI_LINE_STRING | MULTI_LINE_STRING_LITERAL) && string.text().contains('\n') =>
      {
        if let Some(range) = fold_range(mapper, string.text_range(), FoldingRangeKind::Region, true) {
          ranges.push(range);
        }
      }
      _ => {}
...
fn flush_comment_block(mapper: &Mapper, start: &mut Option<TextRange>, end: &mut Option<TextRange>, ranges: &mut Vec<FoldingRange>) {
...
fn fold_range(mapper: &Mapper, range: TextRange, kind: FoldingRangeKind, include_characters: bool) -> Option<FoldingRange> {
  ...
  Some(FoldingRange {
    start_line,
    start_character: include_characters.then(|| u32::try_from(start.character).ok()).flatten(),
    end_line,
    end_character: include_characters.then(|| u32::try_from(end.character).ok()).flatten(),
    kind: Some(kind),
    collapsed_text: None,
  })
}
...
mod tests {
  ...
  fn ranges(source: &str, mapper_source: &str) -> Result<Vec<lsp_types::FoldingRange>, TestFailure> {
    ...
    Ok(create_folding_ranges(syntax, &Mapper::new_utf16(mapper_source, false)))
  }

  /// Project ranges onto the behaviorally relevant line and kind tuple.
  fn line_ranges(ranges: &[lsp_types::FoldingRange]) -> Vec<(u32, u32, Option<FoldingRangeKind>)> {
    ranges
      .iter()
  ...
  fn multiline_values_and_comment_blocks_fold_but_single_lines_do_not() -> Result<(), TestFailure> {
    ...
    let actual = ranges(source, source)?;
      ...
      actual
        .iter()
        .take(2)
        .all(|range| range.start_character.is_some() && range.end_character.is_some()),
  ...
  #[test]
  fn incomplete_or_unmappable_syntax_is_skipped_without_fabricated_ranges() -> Result<(), TestFailure> {
