...
use crate::world::WorkspaceHandle;
use crate::world::World;
use crate::world::send_association_notifications;

#[tracing::instrument(skip_all)]
pub async fn configuration_change<E: Environment>(context: Context<World<E>>, params: Params<DidChangeConfigurationParams>) {
...
pub async fn update_configuration<E: Environment>(mut context: Context<World<E>>) {
  ...
  for root in context.rooted_workspace_urls().await {
    ...
    captured_roots.push((root, ConfigurationItem {
...
fn push_unique_handle<E: Environment>(handles: &mut Vec<WorkspaceHandle<E>>, candidate: WorkspaceHandle<E>) {
  if !handles.iter().any(|existing| Arc::ptr_eq(existing, &candidate)) {
    handles.push(candidate);
  }
}

/// Reinitialize affected workspaces and send all resulting notifications after unlocking them.
async fn reinitialize<E: Environment>(context: Context<World<E>>, handles: Vec<WorkspaceHandle<E>>) {
...
mod tests {
  ...
  #[test]
  fn global_and_scoped_values_apply_by_current_and_captured_identity() -> Result<(), TestFailure> {
    ...
    futures::executor::block_on(async {
      ...
      let detached = world.workspace_for_document(&url("file:///outside/file.toml")?).await;
      ensure(
  ...
  #[test]
  fn malformed_missing_surplus_and_stale_values_leave_unrelated_state_unchanged() -> Result<(), TestFailure> {
    let world = create_world(TestEnvironment::default());
    ...
    futures::executor::block_on(async {
      ...
      let affected = apply_configuration_response(&world, &[captured(url("file:///workspace/live/")?)], &missing).await;
      ensure_eq(
        &affected.len(),
        &2,
