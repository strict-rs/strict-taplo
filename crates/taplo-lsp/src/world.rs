...
use lsp_async_stub::{util::Mapper, Context, RequestWriter};
use regex::Regex;
use serde_json::json;
...
impl<E: Environment> Workspaces<E> {
    ...
    fn insert_root(
        &mut self,
        root: Url,
        handle: WorkspaceHandle<E>,
    ) -> (WorkspaceHandle<E>, bool) {
...
impl<E: Environment> WorldState<E> {
    ...
    pub(crate) async fn workspace_for_document(&self, document: &Url) -> WorkspaceHandle<E> {
        self.workspaces
            .read()
            .await
            .workspace_for_document(document)
    }
    ...
    pub(crate) async fn add_workspace_root(&self, root: Url) -> (WorkspaceHandle<E>, Vec<Url>) {
        ...
        for source in sources {
            ...
            for document_url in document_urls {
                ...
                if Arc::ptr_eq(&owner, &destination)
                    && move_document(&source, &destination, &document_url).await
                {
...
async fn move_document<E: Environment>(
    source: &WorkspaceHandle<E>,
    destination: &WorkspaceHandle<E>,
    document_url: &Url,
) -> bool {
    let document = {
...
impl<E: Environment> WorkspaceState<E> {
    ...
    pub(crate) async fn load_config(
        ...
        let base_path = match &self.root {
            WorkspaceRoot::Rooted(root) => env
                .to_file_path_normalized(root)
        ...
        if self.config.taplo.config_file.enabled {
            let config_path = match &self.config.taplo.config_file.path {
                ...
                Some(path) if matches!(self.root, WorkspaceRoot::Rooted(_)) => {
    ...
    pub(crate) fn document_is_excluded(&self, env: &impl Environment, document: &Url) -> bool {
        ...
        self.taplo_config
            .file_rule
            .as_ref()
            .is_some_and(|rule| !rule.is_match(path))
    }
...
mod tests {
    use super::{
        root_contains_document, DocumentState, WorkspaceRoot, WorkspaceState, Workspaces,
        WorldState,
    };
    ...
    use taplo_common::{config::Config, schema::associations::source, AsyncRwLock};
    use url::Url;
    ...
    fn topology_selects_deepest_root_and_keeps_detached_private() -> Result<(), TestFailure> {
        ...
        let _ = topology.insert_root(nested_url.clone(), nested.clone());
            ...
            Arc::ptr_eq(
                &topology.workspace_for_document(&url("file:///work/nested/file.toml")?),
                &nested,
            ),
            ...
            Arc::ptr_eq(
                &topology.workspace_for_document(&url("file:///work/parent.toml")?),
                &parent,
            ),
            ...
            Arc::ptr_eq(
                &topology.workspace_for_document(&url("file:///elsewhere/file.toml")?),
                &detached,
            ),
    ...
    #[test]
    fn root_changes_migrate_documents_and_recompute_derived_associations(
    ) -> Result<(), TestFailure> {
        let environment = TestEnvironment::default();
        ...
        futures::executor::block_on(async {
            ...
            let migrated_value = ensure_ok(
                serde_json::to_value(&snapshot.document.dom),
                "the migrated DOM must serialize",
            )?;
            ensure_eq(
                &migrated_value,
            ...
            let (duplicate, duplicate_moved) = world.add_workspace_root(nested_url.clone()).await;
                ...
                duplicate_moved.is_empty(),
                "a duplicate root add must not rebalance documents",
            )?;
    ...
    #[test]
    fn workspace_configuration_uses_domain_specific_bases_and_safe_fallbacks(
    ) -> Result<(), TestFailure> {
        let environment = TestEnvironment::default();
        environment.insert_file(
            "/workspace/taplo.toml",
            b"include = [\"**/*.discovered\"]\n".to_vec(),
        );
        environment.insert_file(
            "/configs/absolute.toml",
            b"include = [\"**/*.absolute\"]\n".to_vec(),
        );
        let default_config = Config::default();

        futures::executor::block_on(async {
            let mut detached = WorkspaceState::new(
                environment.clone(),
                WorkspaceRoot::Detached,
                None,
            );
            ...
            environment.insert_file(
                "/workspace/project/taplo.toml",
                b"include = [\"**/*.rooted\"]\n".to_vec(),
            );
            let mut rooted = WorkspaceState::new(
                environment.clone(),
                WorkspaceRoot::Rooted(rooted_url),
                None,
            );
            ensure_anyhow(
            ...
            let mut no_cwd = WorkspaceState::new(
                no_cwd_environment.clone(),
                WorkspaceRoot::Detached,
                None,
            );
            let result = no_cwd
                .load_config(&no_cwd_environment, &default_config)
                .await;
            ensure(
                result.is_err(),
                ...
                !no_cwd.document_is_excluded(
                    &no_cwd_environment,
                    &url("file:///workspace/file.toml")?,
                ),
    ...
    #[test]
    fn offline_workspaces_keep_local_schemas_and_root_relative_associations(
    ) -> Result<(), TestFailure> {
        let environment = TestEnvironment::default();
        let default_config = Config::default();
        ...
        futures::executor::block_on(async {
            ...
            let builtin = ensure_anyhow(
                ...
                builtin.is_object(),
                "the offline built-in schema must remain usable",
            )?;
