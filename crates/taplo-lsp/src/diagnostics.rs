...
pub(crate) async fn publish_diagnostics<E: Environment>(mut context: Context<World<E>>, document_url: Url) {
...
pub(crate) async fn clear_diagnostics<E: Environment>(mut context: Context<World<E>>, document_url: Url) {
...
async fn publish_phase<E: Environment>(context: &mut Context<World<E>>, uri: Uri, diagnostics: Vec<Diagnostic>) {
...
fn collect_dom_errors(document: &DocumentState, document_uri: &Uri) -> Vec<Diagnostic> {
  ...
  for error in errors {
    match &error {
      taplo::dom::Error::ConflictingKeys {
        key,
        other,
      ...
      taplo::dom::Error::UnexpectedSyntax {
        syntax,
      } => {
...
async fn collect_schema_errors<E: Environment>(snapshot: &DocumentSnapshot<E>, document_url: &Url) -> Vec<Diagnostic> {
  ...
  let errors = match snapshot.schemas.validate_root(&association.url, &snapshot.document.dom).await {
    Ok(errors) => errors,
    Err(error) => {
...
mod tests {
  ...
  use super::collect_schema_errors;
